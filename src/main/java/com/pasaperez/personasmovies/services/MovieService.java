...
import com.pasaperez.personasmovies.repositories.ParametroRepository;
...
public class MovieService extends BaseService<Movie, Long> implements IMovieService {
    ...
    public MovieService(BaseRepository<Movie, Long> baseRepository, MovieRepository movieRepository, PersonaRepository personaRepository, ParametroRepository parametroRepository) {
    ...
    public List<Movie> addMovie(Long idPersona, MovieDto movie) throws NotFoundException {
        ...
        Long cantidad = (long) parametroRepository.findById(1L).get().getCantidad();

        if (cantidad>0 && movieRepository.findByPersona(optional.get()).size() < parametroRepository.findById(1L).get().getCantidad()){
            throw new DefaultException("Ya se alcanzo el limite");
        }
